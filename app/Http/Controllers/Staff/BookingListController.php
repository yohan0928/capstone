...
class BookingListController extends Controller
{
    public function showBookingList(Request $request)
    {
        ...
        // Only get the staff's specific branch
        $branchIds = [$staffBranchId];

        // Base query for staff's branch only
        $baseQuery = Booking::where('branch_id', $staffBranchId);
        ...
        if ($request->filled('service_category_id')) {
            ...
            if ($selectedCategory) {
                ...
                $matchingCategoryIds = ServiceCategory::where('branch_id', $staffBranchId)
        ...
        if ($request->filled('service_name_id')) {
            ...
            if ($selectedService) {
                ...
                $matchingServiceIds = ServiceName::where('branch_id', $staffBranchId)
        ...
        // Dropdown options - only staff's branch
        $branches = Branch::where('id', $staffBranchId)
            ->where('active', 1)
            ->orderBy('branch_name')
            ->get();

        // Dynamic dropdowns based on filters - Show unique values
        $categories = ServiceCategory::where('branch_id', $staffBranchId)
            ->where('active', 1)
        ...
        $servicesQuery = ServiceName::where('branch_id', $staffBranchId)
            ->where('active', 1);
        ...
        $seats = Seat::where('branch_id', $staffBranchId)
            ->where('active', 1)
        ...
        $allServicesForCalc = ServiceName::where('branch_id', $staffBranchId)
            ->where('active', 1)
        ...
        if ($request->ajax()) {
            ...
            $bookingsData = $bookings->map(function ($booking) {
                return [
                    ...
                    'unpaid_pay_later_orders_count' => $booking->unpaid_pay_later_orders_count ?? 0,
        ...
        return view('staff.booking.booking_list', compact(
    ...
    /**
     * Get booking orders (unpaid pay-later orders) - MATCHES OWNER SIDE
     */
    public function getBookingOrders(Booking $booking)
    {
        try {
            ...
            // Security check - ensure the booking belongs to staff's branch
    ...
    public function showBookingDetails($booking_uuid)
    {
        ...
        return view('staff.booking.booking_details', compact(
            'booking',
            'mainPayment',
            'extensionPayment',
            'timeUsed',
    ...
    public function updateNote(Request $request)
    {
        ...
        if ($payment->notes) {
            ...
            if (is_string($notesData)) {
                ...
                $existingNotes = (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) ? $decoded : [];
            } elseif (is_array($notesData)) {
        ...
        StaffActivityLogger::logAddNote($booking, $request->note, $request);
            ...
            if ($payment && $payment->notes) {
                ...
                if (is_string($notesData)) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                        ...
                        $notes = [['content' => $notesData, 'added_by' => 'System', 'added_at' => now()->toDateTimeString()]];
                    }
                } elseif (is_array($notesData)) {
                    $notes = $notesData;
                }
                ...
                $notes = array_values(array_filter($notes, function ($note) {
                    return is_array($note) && isset($note['content']) && !empty(trim($note['content']));
                }));
            }
    ...
    public function confirmBooking(Request $request)
    {
        ...
        if ($booking->booking_status == 2 && $booking->booking_type == 1) {
            ...
            StaffActivityLogger::logConfirmBooking($booking, $request);
    ...
    public function sendQRCodeEmail($customer, $booking, $serviceName, $seat)
    {
        ...
        try {
            ...
            $emailData = [
                ...
                'qrCodeAttached' => $qrCodeAttached,
            ];

            try {
                Mail::send('staff.booking.send_confirmation_email_qr_code', $emailData, function ($message) use ($customer, $booking, $tempPath) {
                    $message
                        ->to($customer->email)
    ...
    private function sendEmailWithoutQRCode($customer, $booking, $serviceName, $seat)
    {
        try {
            ...
            Mail::send('staff.booking.send_confirmation_email_qr_code', $emailData, function ($message) use ($customer, $booking) {
                $message
                    ->to($customer->email)
                    ->subject('From LinkudHub')
                    ->from(config('mail.from.address'), config('mail.from.name'));
    ...
    public function markNoShow(Request $request)
    {
        ...
        $staff = Auth::guard('staff')->user();
        $ownerId = $staff->owner_account_id;
        $branch = Branch::where('owner_account_id', $ownerId)->pluck('id');
        ...
        if ($booking->booking_status == 1) {
            ...
            StaffActivityLogger::logMarkNoShow($booking, $request);
