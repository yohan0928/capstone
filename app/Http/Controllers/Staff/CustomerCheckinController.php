...
class CustomerCheckinController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Get customer checkins with related data and pagination - ONLY staff's branch
        $query = CustomerCheckin::with([
            ...
            ->where('branch_id', $branchId)  // Only staff's branch
            ->where('active', 1);
        ...
        // Get statistics for the dashboard cards - ONLY staff's branch
        $statsQuery = CustomerCheckin::where('branch_id', $branchId)
            ->where('active', 1);
        ...
        return view('staff.booking.customer_checkins', compact('customerCheckins', 'stats', 'scannedBrn'));
    }
    ...
    public function showExtendTimeModal($id)
    {
        try {
            ...
            // Verify staff access
            $this->verifyStaffAccess($customerCheckin);
            ...
            // Get existing bookings for the same seat/branch to prevent overlap - ADDED THIS
            ...
            // Calculate maximum available duration considering existing bookings - ADDED THIS
            ...
            $response = [
                ...
                'existing_bookings' => $existingBookings, // ADDED THIS
                'max_available_duration' => $maxAvailableDuration, // in minutes - ADDED THIS
            ];
    ...
    /**
     * Get existing bookings for the same seat to prevent time overlap - ADDED THIS METHOD
     */
    ...
    /**
     * Calculate maximum available duration considering branch closing time and existing bookings - ADDED THIS METHOD
     */
    ...
    public function extendTime(Request $request, $id)
    {
        ...
        try {
            ...
            // Verify the staff has access to this checkin
            $this->verifyStaffAccess($customerCheckin);

            $booking = $customerCheckin->booking;
            $staff = Auth::guard('staff')->user();
            $customer = $customerCheckin->customerAccount;
            $branch = $customerCheckin->branch;

            // Capture original times for logging
            $originalEndTime = $booking->extended_end_time ?? $booking->end_time;
            $originalDateEnd = $booking->extended_date_end ?? $booking->date_end;

            // Check for existing bookings that would overlap with the extension - ADDED THIS
            $existingBookings = $this->getExistingBookingsForSeat(
                $customerCheckin->branch_id,
                $customerCheckin->seat_id,
            ...
            // Check for conflicts - ADDED THIS
            ...
            // Add audit trail to checkin record
            if (!is_null($customerCheckin->updated_by)) {
                $customerCheckin->last_updated_by = $customerCheckin->updated_by;
                $customerCheckin->last_updated_by_type = $customerCheckin->updated_by_type;
                $customerCheckin->last_date_updated = $customerCheckin->date_updated;
            }
            $customerCheckin->updated_by = $staff->id;
            $customerCheckin->updated_by_type = 'staff';
            $customerCheckin->date_updated = now();

            // If checkin doesn't have creation data, set it
            if (!$customerCheckin->created_by) {
                $customerCheckin->created_by = $staff->id;
                $customerCheckin->created_by_type = 'staff';
            ...
            // LOG: EXTEND TIME ACTION
            StaffActivityLogger::logExtendTime(
                $booking,
                $customer,
                [
                    'original_end_time' => $originalEndTime,
                    'new_end_time' => $booking->extended_end_time,
                    'additional_duration' => $additionalDuration,
                    'additional_duration_formatted' => $this->formatDuration($additionalDuration),
                    'total_extended_minutes' => $customerCheckin->extended_time_used,
                    'is_first_extension' => $isFirstExtension,
                    'new_date_end' => $booking->extended_date_end,
            ...
            // Send notifications - FIXED: Use staff guard instead of owner
            $actor = Auth::guard('staff')->user();
            $owner = OwnerAccount::find($staff->owner_account_id);
    ...
    /**
     * Verify that the authenticated staff has access to the checkin record (same branch)
     */
    private function verifyStaffAccess(CustomerCheckin $customerCheckin)
    {
        $staff = Auth::guard('staff')->user();

        if ($customerCheckin->branch_id !== $staff->branch_id) {
            abort(403, 'Unauthorized access to this checkin record');
        }
    }

    /**
     * Check if time slot is available (doesn't conflict with existing bookings) - KEPT BUT NOT USED IN MODAL
     */
