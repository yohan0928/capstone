...
class BookingListPaymentController extends Controller
{
    ...
    public function showMainPaymentPage($booking_uuid)
    {
        ...
        // Security check - ensure booking belongs to staff's branch
        ...
        // Additional security check - ensure staff's branch belongs to their owner
        $staffBranch = Branch::find($staffBranchId);
        if (!$staffBranch || $staffBranch->owner_account_id !== $ownerId) {
            abort(403, 'Unauthorized access to this booking.');
        }
        ...
        $formattedData['has_unpaid_orders'] = $hasUnpaidOrders;

        return view('staff.booking.booking_list_payment.main_payment_page', compact(
            'booking', 'checkin', 'totalTimeUsed', 'extendedTimeUsed',
            'formattedData', 'mainPayment', 'mainAmount', 'hourlyRate', 'fifteenMinRate',
            'unpaidOrders', 'totalUnpaidOrdersAmount', 'grandTotal', 'hasUnpaidOrders'
        ));
    }
    ...
    public function showExtensionPaymentPage($booking_uuid)
    {
        ...
        // Get breakdown for table display
        ...
        $formattedData = [
            ...
            // Duration
            'total_time_used_formatted' => $this->formatDuration($totalTimeUsed),
            'extended_time_formatted' => $this->formatExtensionTime($extendedTimeUsed),
        ...
        return view('staff.booking.booking_list_payment.extension_payment_page', compact(
    ...
    public function showOrderPaymentPage($booking_uuid)
    {
        ...
        if (!$hasUnpaidOrders) {
            return redirect()
                ->route('sub_two.booking_lists.showBookingList')
                ->with('error', 'No unpaid orders found for this booking.');
        }

        return view('staff.booking.booking_list_payment.order_payment_page', compact(
            'booking',
            'unpaidOrders',
    ...
    public function updateOrderPayment(Request $request)
    {
        ...
        foreach ($unpaidOrders as $order) {
            ...
            foreach ($unpaidPayments as $payment) {
                ...
                $oldPaymentStatus = $payment->order_payment_status;
                ...
                $payment->updated_by = Auth::guard('staff')->id();
                $payment->updated_by_type = 'staff';
                $payment->date_updated = now();

                if (!$payment->created_by) {
                    $payment->created_by = Auth::guard('staff')->id();
                    $payment->created_by_type = 'staff';
                ...
                // LOG THE ORDER PAYMENT ACTION
                StaffActivityLogger::logProcessOrderPayment($order, [
                    'order_id' => $order->id,
                    ...
                    'status' => 'paid',
                    'previous_status' => $oldPaymentStatus == 1 ? 'paid' : 'unpaid'
                ]);
            }
            ...
            $order->updated_by = Auth::guard('staff')->id();
            $order->updated_by_type = 'staff';
            $order->date_updated = now();

            if (!$order->created_by) {
                $order->created_by = Auth::guard('staff')->id();
                $order->created_by_type = 'staff';
        ...
        if ($paymentSuccess) {
            ...
            if ($allPaymentsPaid) {
                ...
                $booking->updated_by = Auth::guard('staff')->id();
                $booking->updated_by_type = 'staff';
                $booking->date_updated = now();
                
                // If booking doesn't have creation data, set it
                if (!$booking->created_by) {
                    $booking->created_by = Auth::guard('staff')->id();
                    $booking->created_by_type = 'staff';
        ...
        return redirect()
            ->route('sub_two.booking_lists.showBookingList', ['brn' => $booking->booking_ref_no])
    ...
    private function getExtensionBreakdown($booking, $extendedTimeUsed, $extendedAmount, $fifteenMinRate)
    {
        ...
        if ($extendedHours <= 1) {
            ...
            $breakdown[] = [
                ...
                'time' => $this->formatExtensionTime($extendedTimeUsed),
            ...
            if ($minutesBeyondPackage > 0) {
                ...
                $breakdown[] = [
                    'label' => 'Extra Time (15-min increments)',
                    'time' => $this->formatExtensionTime($minutesBeyondPackage),
                    'price' => $this->formatCurrency($extraAmount)
                ];
            }
        }
    ...
    private function calculateExtendedTimeAmountNew($booking, $extendedTimeUsed)
    {
        ...
        // Pricing logic using only 15-min rate
    ...
    public function updateMainPayment(Request $request)
    {
        ...
        if (!$mainPayment) {
            ...
            if (!$mainPayment->created_by) {
                $mainPayment->created_by = Auth::guard('staff')->id();
                $mainPayment->created_by_type = 'staff';
        ...
        // Capture old status for logging
        $oldPaymentStatus = $mainPayment->payment_status;
        $oldPaymentMethod = $mainPayment->payment_method ?? 'none';
        $oldTotalAmount = $mainPayment->total_amount ?? 0;
        ...
        $mainPayment->updated_by = Auth::guard('staff')->id();
        $mainPayment->updated_by_type = 'staff';
        ...
        // LOG THE MAIN PAYMENT ACTION
        StaffActivityLogger::logProcessMainPayment($booking, [
            'method' => $request->payment_method,
            'total_amount' => $grandTotal,
            'amount_paid' => $mainPayment->amount_paid,
            'change' => $mainPayment->change,
            'status' => $mainPayment->payment_status == 1 ? 'paid' : 'unpaid',
            'previous_status' => $oldPaymentStatus == 1 ? 'paid' : 'unpaid',
            'unpaid_orders_count' => $unpaidOrders->count(),
            'unpaid_orders_amount' => $totalUnpaidOrdersAmount,
        ...
        Notification::send($staffs, new BookingListPaymentStaffNotification(
            $booking,
            $bookingBranch,
            $customer,
            $actor,
    ...
    public function updateExtensionPayment(Request $request)
    {
        ...
        if (!$extensionPayment) {
            ...
            if (!$extensionPayment->created_by) {
                $extensionPayment->created_by = Auth::guard('staff')->id();
                $extensionPayment->created_by_type = 'staff';
        ...
        // Capture old status for logging
        $oldPaymentStatus = $extensionPayment->payment_status;
        $oldPaymentMethod = $extensionPayment->payment_method ?? 'none';
        $oldTotalAmount = $extensionPayment->total_amount ?? 0;
        ...
        $extensionPayment->updated_by = Auth::guard('staff')->id();
        $extensionPayment->updated_by_type = 'staff';
        ...
        // LOG THE EXTENSION PAYMENT ACTION
        StaffActivityLogger::logProcessExtensionPayment($booking, [
            'method' => $request->payment_method,
            'extended_time' => $extendedTimeUsed,
            'extended_time_formatted' => $this->formatExtensionTime($extendedTimeUsed),
            'extension_amount' => $extendedAmount,
            'total_amount' => $grandTotal,
            'amount_paid' => $extensionPayment->amount_paid,
            'change' => $extensionPayment->change,
            'status' => $extensionPayment->payment_status == 1 ? 'paid' : 'unpaid',
            'previous_status' => $oldPaymentStatus == 1 ? 'paid' : 'unpaid',
            'unpaid_orders_count' => $unpaidOrders->count(),
            'unpaid_orders_amount' => $totalUnpaidOrdersAmount,
            'grand_total' => $grandTotal
    ...
    private function updateOrderPayments($unpaidOrders, $paymentMethod)
    {
        foreach ($unpaidOrders as $order) {
            ...
            foreach ($unpaidPayments as $payment) {
                // Capture old status for logging
                $oldPaymentStatus = $payment->order_payment_status;
                $oldPaymentMethod = $payment->payment_method ?? 'none';
            ...
            // Update order status to paid (1 = ordered/paid)
