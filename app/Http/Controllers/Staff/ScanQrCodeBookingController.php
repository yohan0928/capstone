...
class ScanQrCodeBookingController extends Controller
{
    ...
    {
        return view('staff.booking.scan_qr_code_booking');
    }
    ...
    public function redirectToPos(Request $request)
    {
        try {
            ...
            // Verify staff has access to this booking
            $staff = Auth::guard('staff')->user();
            if ($booking->branch_id != $staff->branch_id) {
                return response()->json([
                    'success' => false,
            ...
            $posUrl = route('sub_two.pos.index');
            $params = [];
        ...
        try {
            ...
            return response()->json([
                ...
                'booking' => [
                    ...
                    'main_notes' => $mainPayment ? $mainPayment->notes : null,
                    'extended_notes' => $extendedPayment ? $extendedPayment->notes : null,
    ...
    // This function is only intended for ONLINE BOOKING
    public function storeCheckin(Request $request)
    {
        try {
            ...
            StaffActivityLogger::logCheckinCustomer(
                $booking,
                $booking->customerAccount,
                [
                    'checkin_time' => now('Asia/Manila')->format('Y-m-d H:i:s'),
                    'service_name' => $booking->serviceName ? $booking->serviceName->service_name : 'N/A',
                    'room_seat' => $booking->seat ? ($booking->seat->room_no ?: $booking->seat->seat_no) : 'N/A',
            ...
            // Send notification
            Notification::send($staffs, new ScanQrCodeBookingStaffNotification(
                $booking,
                $bookingBranch,
                $customer,
                $actor,
            ...
            return response()->json([
                ...
                'redirect_url' => route('sub_two.customer_checkins.index') . '?brn=' . urlencode($booking->booking_ref_no)
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
    ...
    public function checkout(Request $request)
    {
        try {
            ...
            $staff = Auth::guard('staff')->user();
            if ($checkin->branch->owner_account_id != $staff->owner_account_id || $checkin->branch_id != $staff->branch_id) {
                return response()->json([
                    'success' => false,
            ...
            if ($hasExtendedTime) {
                ...
                $checkin->booking->updated_by = Auth::guard('staff')->id();
                $checkin->booking->updated_by_type = 'staff';
            ...
            StaffActivityLogger::log(
                StaffActivityLog::ACTION_COMPLETE_BOOKING,
                "Checked out customer for booking #{$checkin->booking->booking_ref_no}",
                $checkin->booking->id,
                [
                    'booking_ref_no' => $checkin->booking->booking_ref_no,
                    'customer_id' => $checkin->customer_account_id,
                    'customer_name' => $checkin->customerAccount ? "{$checkin->customerAccount->first_name} {$checkin->customerAccount->last_name}" : 'N/A',
                    'checkin_id' => $checkin->id,
                    'time_used' => $checkin->time_used,
                    'extended_time_used' => $checkin->extended_time_used ?? 0,
                    'total_time_used' => $checkin->total_time_used,
                    'booking_type' => $checkin->booking->booking_type == 0 ? 'walk-in' : 'online',
            ...
            $owners = OwnerAccount::where('id', $staffUser->owner_account_id)->get();
            ...
            if ($hasUnpaidOrders) {
                $redirectUrl = route('sub_two.booking_lists.order_payment', ['booking_uuid' => $booking->uuid]);
                $paymentType = 'order';
            } elseif ($hasExtendedTimeUsed && !$hasExtensionPayment) {
                $redirectUrl = route('sub_two.booking_lists.extension_payment', ['booking_uuid' => $booking->uuid]);
                $paymentType = 'extension';
            } elseif (!$hasMainPayment) {
                $redirectUrl = route('sub_two.booking_lists.main_payment', ['booking_uuid' => $booking->uuid]);
                $paymentType = 'main';
            } else {
                $redirectUrl = route('sub_two.booking_lists.showBookingList');
                $paymentType = 'none';
            }
