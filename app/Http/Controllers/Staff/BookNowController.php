...
use App\Notifications\Customer\BookNowCustomerNotification;
...
class BookNowController extends Controller
{
    public function create()
    {
        ...
        $serviceNames = ServiceName::where('active', 1)
            ->where('owner_account_id', $ownerId)
            ->whereHas('serviceCategory', function ($query) use ($branchId, $ownerId) {
        ...
        // Get loyalty tiers for reward display
        $loyaltyTiers = LoyaltyTier::where('active', 1)
            ->where('owner_account_id', $ownerId)
            ->where('branch_id', $branchId)
            ->get();

        return view('staff.booking.book_now', compact(
            'branch',
            'serviceCategories',
            'serviceNames',
            'seats',
    ...
    public function store(Request $request)
    {
        ...
        if ($paymentMethod !== 3) {  // Not pay later
            if (empty($validated['amount_paid']) || $validated['amount_paid'] < $validated['total_amount']) {
        ...
        return DB::transaction(function () use ($validated, $paymentMethod, $request) {
            $customer = null;
            $isNewCustomer = false;
            ...
            // Get room/seat info for logging
            $roomSeatInfo = '';
            if ($seat) {
                $parts = [];
                if (!empty($seat->room_no)) {
                    $parts[] = 'Room ' . $seat->room_no;
                }
                if (!empty($seat->seat_no)) {
                    $parts[] = 'Seat ' . $seat->seat_no;
                }
                $roomSeatInfo = count($parts) > 0 ? implode(' / ', $parts) : 'N/A';
            }

            // Create Booking with conditional creation data
            $bookingData = [
            ...
            // LOG: CREATE BOOKING - After booking is saved with real ID
            StaffActivityLogger::logCreateBooking(
                $booking,
                $customer,
                [
                    'service_name' => $serviceName->service_name ?? 'N/A',
                    'service_price' => $validated['total_amount'],
                    'room_seat' => $roomSeatInfo,
                    ...
                    'payment_status' => $paymentMethod == 3 ? 'unpaid' : 'paid',
            ...
            // Create Booking Payment with conditional creation data
            ...
            $paymentData = [
                ...
                'change' => $change,
                'payment_category' => 1,  // 1=main(main payment)
            ...
            if (!$bookingPayment->created_by) {
                $bookingPayment->created_by = Auth::guard('staff')->id();
                $bookingPayment->created_by_type = 'staff';
                $bookingPayment->date_created = now();
            }

            $bookingPayment->save();

            // LOG: CHECKIN CUSTOMER
            StaffActivityLogger::logCheckinCustomer(
                $booking,
                $customer,
                [
            ...
            // Create Customer Checkin Record with conditional creation data
            $checkinData = [
                'customer_account_id' => $customer->id,
                'branch_id' => $validated['branch_id'],
                'service_category_id' => $validated['service_category_id'],
            ...
            if (!$customerCheckin->created_by) {
                $customerCheckin->created_by = Auth::guard('staff')->id();
    ...
    if ($customerReward && $customerReward->customer_account_id == $customer->id) {
        ...
        \Log::info('Reward redeemed successfully by staff', [
            'customer_reward_id' => $customerReward->id,
            'booking_id' => $booking->id,
            ...
            // Send notification
            Notification::send($staffs, new BookNowStaffNotification(
                $booking,
                $bookingBranch,
                $customer,
                $actor,
            ...
            Notification::send($customer, new BookNowCustomerNotification(
                $booking,
                $bookingBranch,
                $customer,
                $actor,
            ...
            return redirect()
                ->route('sub_two.customer_checkins.index', ['brn' => $bookingRefNo])
    ...
    public function sendQRCodeEmail($customer, $booking, $serviceName, $seat, $isNewCustomer = false, $generatedPassword = null)
    {
        ...
        try {
            ...
            try {
                Mail::send('staff.booking.send_checkout_email_qr_code', $emailData, function ($message) use ($customer, $booking, $tempPath) {
                    $message
                        ->to($customer->email)
    ...
    private function sendEmailWithoutQRCode($customer, $booking, $serviceName, $seat, $isNewCustomer = false, $generatedPassword = null)
    {
        try {
            ...
            Mail::send('staff.booking.send_checkout_email_qr_code', $emailData, function ($message) use ($customer, $booking) {
                $message
                    ->to($customer->email)
                    ->subject('From LinkudHub')
                    ->from(config('mail.from.address'), config('mail.from.name'));
