...
{
    /**
     * Display a listing of redeemable items.
     */
    public function index(Request $request)
    {
        ...
        // Status filter (using both active and is_active)
        ...
        // Calculate stats
        $stats = [
            'total' => RedeemableItem::where('owner_account_id', $ownerId)->whereNull('deleted_at')->count(),
        ...
        // Get services with their categories and branches
        ...
        // Get products with their branches and ingredients
        ...
        return view('owner.loyalty.redeemable_items', compact('items', 'branches', 'services', 'products', 'stats'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $item = RedeemableItem::where('owner_account_id', $owner->id)
                ->whereNull('deleted_at')
                ->findOrFail($id);
    ...
    public function getServicePrice($id)
    {
        try {
            ...
            $service = ServiceName::where('owner_account_id', $owner->id)
                ->where('active', 1)
                ->findOrFail($id);
    ...
    public function getProductPrice($id)
    {
        try {
            ...
            $product = Product::where('owner_account_id', $owner->id)
                ->where('active', 1)
                ->findOrFail($id);
    ...
    /**
     * Bulk update monetary values for items
     */
    public function bulkUpdateMonetaryValues(Request $request)
    {
        try {
            $owner = Auth::guard('owner')->user();
            ...
            $validated = $request->validate([
                ...
                'items.*.id' => 'required|exists:redeemable_items,id',
                'items.*.monetary_value' => 'nullable|numeric|min:0'
            ]);
            ...
            foreach ($validated['items'] as $itemData) {
                ...
                if ($item && in_array($item->reward_type, ['free_service', 'free_product', 'fixed_discount'])) {
                    $item->update([
                        'monetary_value' => $itemData['monetary_value'],
                        'last_updated_by' => auth()->id(),
                        'last_updated_by_type' => 'owner',
                        'last_date_updated' => now()
                    ]);
            ...
            DB::rollBack();
            Log::error('Error bulk updating monetary values: ' . $e->getMessage());
            return response()->json([
                'success' => false,
