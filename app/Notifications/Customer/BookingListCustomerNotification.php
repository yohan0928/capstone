...
class BookingListCustomerNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'url' => route('sub_three.my_bookings.showMyBookings', ['brn' => $this->booking->booking_ref_no]),
        ];
    }
    ...
    private function getMessage(): string
    {
        ...
        switch ($this->action) {
            case 'notes':
                return "{$actorName} added a new NOTE: {$bookingRefNo} at {$branch}";

            case 'confirmed':
                return "{$bookingRefNo} | Your booking is CONFIRMED at {$branch}";

            case 'no_show':
                return "{$bookingRefNo} | Your booking marked as NO SHOW at {$branch}";

            case 'feedback':
                $ratingStars = str_repeat('★', $this->additionalData['rating'] ?? 0)
                    . str_repeat('☆', 5 - ($this->additionalData['rating'] ?? 0));
                return "You submitted feedback at {$branch} - Rating: {$ratingStars}";

            default:
                return "Someone performed action: {$bookingRefNo} at {$branch}";
        }
    }
