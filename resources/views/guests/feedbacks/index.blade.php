...
@section('content')
    ...
    <section class="py-6 bg-white">
        <div class="container mx-auto px-3">
            ...
            @php
                // Defensive defaults: guarantees keys always exist even if
                // HomeController::getAverageRatings() omits an empty group.
                $ratingCategories = $averageRatings['service_categories'] ?? collect();
                $ratingServices   = $averageRatings['service_names'] ?? collect();
                $ratingBranches   = $averageRatings['branches'] ?? collect();
            @endphp
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                ...
                <div
                    ...
                    <div class="space-y-3">
                        @forelse ($ratingCategories->take(3) as $category)
                            @php
                                // Support both object (stdClass/Eloquent) and array shapes
                                $categoryData = is_array($category) ? (object) $category : $category;
                            @endphp
                            <div
                                class="flex items-center justify-between group hover:bg-[#f5f0eb] p-2 rounded transition-colors">
                                <div class="flex-1 min-w-0">
                                    <span
                                        class="text-xs text-gray-700 font-medium block truncate">{{ $categoryData->service_category }}</span>
                                </div>
                                <div class="flex items-center ml-2">
                                    <div class="flex text-yellow-500 mr-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <i
                                                class="fas fa-star {{ $i <= round($categoryData->average_rating) ? 'text-yellow-500' : 'text-gray-300' }} text-xs"></i>
                                        @endfor
                                    </div>
                                    <span
                                        class="text-xs font-medium text-[#4a3429] ml-1">{{ number_format($categoryData->average_rating, 1) }}</span>
                                    <span class="text-xs text-gray-500 ml-1">({{ $categoryData->review_count }})</span>
                                </div>
                            </div>
                        @empty
                            <p class="text-xs text-gray-400 text-center py-2">No category ratings yet</p>
                        @endforelse
                        @if ($ratingCategories->count() > 3)
                            <div class="text-center pt-2">
                                <a href="#reviews" class="text-[#7f5539] hover:text-[#6b4f3c] text-xs font-medium">
                                    View all {{ $ratingCategories->count() }} categories
                                </a>
                            </div>
                        @endif
                    </div>
                ...
                <div
                    ...
                    <div class="space-y-3">
                        @forelse ($ratingServices->take(3) as $service)
                            @php
                                $serviceData = is_array($service) ? (object) $service : $service;
                            @endphp
                            <div
                                class="flex items-center justify-between group hover:bg-[#f5f0eb] p-2 rounded transition-colors">
                                <div class="flex-1 min-w-0">
                                    <span
                                        class="text-xs text-gray-700 font-medium block truncate">{{ $serviceData->service_name }}</span>
                                </div>
                                <div class="flex items-center ml-2">
                                    <div class="flex text-yellow-500 mr-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <i
                                                class="fas fa-star {{ $i <= round($serviceData->average_rating) ? 'text-yellow-500' : 'text-gray-300' }} text-xs"></i>
                                        @endfor
                                    </div>
                                    <span
                                        class="text-xs font-medium text-[#4a3429] ml-1">{{ number_format($serviceData->average_rating, 1) }}</span>
                                    <span class="text-xs text-gray-500 ml-1">({{ $serviceData->review_count }})</span>
                                </div>
                            </div>
                        @empty
                            <p class="text-xs text-gray-400 text-center py-2">No service ratings yet</p>
                        @endforelse
                        @if ($ratingServices->count() > 3)
                            <div class="text-center pt-2">
                                <a href="#reviews" class="text-[#7f5539] hover:text-[#6b4f3c] text-xs font-medium">
                                    View all {{ $ratingServices->count() }} services
                                </a>
                            </div>
                        @endif
                    </div>
                ...
                <div
                    ...
                    <div class="space-y-3">
                        @forelse ($ratingBranches->take(3) as $branch)
                            @php
                                $branchData = is_array($branch) ? (object) $branch : $branch;
                            @endphp
                            <div
                                class="flex items-center justify-between group hover:bg-[#f5f0eb] p-2 rounded transition-colors">
                                <div class="flex-1 min-w-0">
                                    <span
                                        class="text-xs text-gray-700 font-medium block truncate">{{ $branchData->branch_name }}</span>
                                </div>
                                <div class="flex items-center ml-2">
                                    <div class="flex text-yellow-500 mr-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <i
                                                class="fas fa-star {{ $i <= round($branchData->average_rating) ? 'text-yellow-500' : 'text-gray-300' }} text-xs"></i>
                                        @endfor
                                    </div>
                                    <span
                                        class="text-xs font-medium text-[#4a3429] ml-1">{{ number_format($branchData->average_rating, 1) }}</span>
                                    <span class="text-xs text-gray-500 ml-1">({{ $branchData->review_count }})</span>
                                </div>
                            </div>
                        @empty
                            <p class="text-xs text-gray-400 text-center py-2">No branch ratings yet</p>
                        @endforelse
                        @if ($ratingBranches->count() > 3)
                            <div class="text-center pt-2">
                                <a href="#reviews" class="text-[#7f5539] hover:text-[#6b4f3c] text-xs font-medium">
                                    View all {{ $ratingBranches->count() }} branches
                                </a>
                            </div>
                        @endif
                    </div>
                            ...
                            <div class="mb-3">
                                <div class="flex flex-wrap gap-1 mb-2">
                                    <span
                                        ...
                                        <i class="fas fa-tag mr-1"></i>{{ $feedback->serviceCategory->service_category ?? 'N/A' }}
                                    </span>
                                    <span
                                        class="inline-block bg-[#e6f7f0] text-[#2e8b57] text-xs px-2 py-0.5 rounded border border-[#d1f0e1]">
                                        <i
                                            class="fas fa-concierge-bell mr-1"></i>{{ $feedback->serviceName->service_name ?? 'N/A' }}
                                    </span>
                                    <span
                                        class="inline-block bg-[#f0e6f7] text-[#7b3f9e] text-xs px-2 py-0.5 rounded border border-[#e6d1f0]">
                                        <i class="fas fa-store mr-1"></i>{{ $feedback->branch->branch_name ?? 'N/A' }}
                                    </span>
                                </div>
                            </div>
