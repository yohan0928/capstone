...
<div class="flex items-center justify-center p-4">
    <div class="relative bg-white shadow-md rounded-lg w-full max-w-2xl p-8 border border-[#4A2C1D]">
        ...
        <form action="{{ route('sub_two.product_ingredients.updateProductIngredient', $productIngredient->uuid) }}"
            method="POST" enctype="multipart/form-data" class="space-y-6"
            x-data="ingredientCalculator({{ Js::from($ingredients) }}, {{ Js::from($productIngredient) }})"
            x-init="initEdit()">
...
<script>
function ingredientCalculator(ingredients, existing) {
    return {
        selectedIngredientId: existing.ingredient_id || '',
        selectedUnit: existing.unit || '',
        quantityNeeded: existing.quantity_needed || 0,
        convertedQuantity: null,
        totalStockInBase: null,
        baseUnit: '', // No default value
        ...
        selectIngredient(id) {
            ...
            this.selectedUnit = '';
            this.baseUnit = this.selectedIngredient?.converted_unit || '';
        ...
        computeConversion() {
            ...
            // Set base unit from ingredient's converted_unit
            this.baseUnit = ingredient.converted_unit || '';

            const conversions = ingredient.unit_conversions || {};

            // Convert quantity to base unit
            this.convertedQuantity = this.quantityNeeded * (conversions[this.selectedUnit] || 1);
            
            // Convert stock quantity to base unit
            const stockConversionFactor = conversions[ingredient.unit] || 1;
            this.totalStockInBase = (ingredient.converted_stock_quantity_in ?? ingredient.stock_quantity_in ?? 0) * stockConversionFactor;
        },

        initEdit() {
            // Set base unit from existing ingredient data
            if (this.selectedIngredientId) {
                const ingredient = this.selectedIngredient;
                this.baseUnit = ingredient?.converted_unit || '';
            }

            if (this.selectedIngredientId && this.selectedUnit && this.quantityNeeded > 0) {
                this.computeConversion();
            }
        }
    }
