...
@section('content')
    <div x-data="ingredientData()" x-init="init()" class="p-4">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex-1 text-right">
                <a href="{{ route('sub_two.ingredients.showDeactivatedIngredient') }}"
        ...
        <!-- Stats Grid -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
            <!-- Total Ingredients -->
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900" x-text="stats.low_stock_ingredients"></p>
                    </div>
                            ...
                            <tr :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-100'">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">
                                        <span
                                            x-text="formatNumber(ingredient.stock_quantity_in) + ' ' + ingredient.unit"></span>
                                    </div>
                                    <div class="text-sm text-gray-500">
                                        <span
                                            x-text="'Threshold: ' + formatNumber(ingredient.stock_quantity_threshold) + ' ' + ingredient.unit"></span>
                                    </div>
                                    <template x-if="ingredient.stock_quantity_in <= ingredient.stock_quantity_threshold">
                                        <span
                                ...
                                <!-- Expiration -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <template x-if="ingredient.date_expiration">
                                        <div>
                                            <div class="text-sm text-gray-900"
                                                x-text="formatDate(ingredient.date_expiration)"></div>
                                            <div class="text-sm text-gray-500"
                                                x-text="formatTime(ingredient.date_expiration)"></div>
                                        </div>
                                    </template>
                                    <template x-if="!ingredient.date_expiration">
                                    ...
                                    <div x-data="{ open: false }" class="relative">
                                        ...
                                        <div x-show="open"
                                            ...
                                            <form :id="'update-ingredient-status-' + ingredient.uuid + '-available'"
                                                :action="'{{ url('sub_two/ingredients/status') }}/' + ingredient.uuid"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <form id="addIngredientForm" @submit.prevent="submitAddForm" enctype="multipart/form-data">
                        ...
                        <!-- Hidden branch_id field (Staff's branch) -->
                        ...
                        <div class="space-y-4 overflow-y-auto pr-2">
                            {{-- Ingredient Image --}}
                            <div>
                                <label class="block text-sm font-medium text-[#4A2C1D]">Ingredient Image</label>
                                <div class="my-2">
                            ...
                            <div x-data="addConversionData()">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-[#4A2C1D]">Stock Quantity</label>
                                        <input type="number" name="stock_quantity_in" min="0"
                                ...
                                {{-- Stock Threshold --}}
                                <div>
                                    <label class="block text-sm font-medium text-[#4A2C1D]">Stock Quantity
                                        Threshold</label>
                ...
                <div
                    ...
                    <template x-if="editIngredientData">
                        <form id="editIngredientForm" @submit.prevent="submitEditForm" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="branch_id" :value="editIngredientData.branch_id">
                            ...
                            <div class="space-y-4 overflow-y-auto pr-2">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-[#4A2C1D]">Expiration Date
                                        (Optional)</label>
                                    <input type="date" name="date_expiration"
                                        x-model="editIngredientData.date_expiration"
                                        class="w-full border-2 border-[#7F5539] rounded px-3 py-2 mt-1">
                                </div>
                            </div>
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('editConversionData', () => ({
                ...
                initEditData() {
                    ...
                    if (mainComponent.editIngredientData) {
                        this.stockQty = mainComponent.editIngredientData.stock_quantity_in || '';
            ...
            Alpine.data('ingredientData', () => ({
                ...
                async submitAddForm() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            '{{ route('sub_two.ingredients.storeIngredientAjax') }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'Accept': 'application/json',
                ...
                async openEditModal(ingredient) {
                    ...
                    try {
                        const response = await fetch(
                            `{{ url('sub_two/ingredients') }}/${ingredient.uuid}/data`, {
                                method: 'GET',
                                headers: {
                                    'Accept': 'application/json',
                                    'X-Requested-With': 'XMLHttpRequest'
    ...
    try {
        ...
        const endpoint = `{{ route('sub_two.ingredients.updateIngredientAjax', ['ingredient_uuid' => ':uuid']) }}`
            .replace(':uuid', this.editIngredientData.uuid);

        // Fire and forget - we'll refresh regardless
        fetch(endpoint, {
            method: 'POST',
            body: formData,
            headers: {
            ...
            console.log('Update request may have failed');
        });
        ...
        // Small delay before refreshing to ensure server processes request
        setTimeout(async () => {
            await this.applyFilters(this.currentFilters);
            this.isSubmitting = false;
        }, 300);
                ...
                confirmArchive() {
                    ...
                    if (quantityOut > 0) {
                        ...
                        actionUrl =
                            `{{ url('sub_two/ingredients/damage') }}/${this.selectedIngredient.uuid}`;
                    } else {
                        // Call regular deactivate/archive endpoint
                        actionUrl =
                            `{{ url('sub_two/ingredients/deactivate') }}/${this.selectedIngredient.uuid}`;
                    }
