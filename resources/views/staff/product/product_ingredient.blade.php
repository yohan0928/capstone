                ...
                <div
                    ...
                    <div>
                        ...
                        <template x-if="editIngredientData">
                            <form id="editProductIngredientForm" @submit.prevent="submitEditForm" x-data="editIngredientCalculator(editIngredientData)">
                                @csrf
                                @method('PATCH')
                                <input type="hidden" name="product_id" value="{{ $products->id }}">
                                <input type="hidden" name="branch_id" value="{{ $branches->id }}">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('ingredientCalculator', () => ({
                ...
                // Filter ingredients by product's branch_id (staff can only see ingredients from their branch)
            ...
            Alpine.data('productIngredientData', () => ({
                ...
                shouldHidePageNumber(page) {
                    ...
                    if (Math.abs(page - current) <= 1) return false;
                    return true;
                },
                ...
                async openEditModal(ingredient) {
                    ...
                    try {
                        const response = await fetch(
                            `{{ url('sub_two/product_ingredients/data') }}/{{ $products->uuid }}/${ingredient.uuid}`, {
                                method: 'GET',
                                headers: {
                ...
                async submitAddForm() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            '{{ route("sub_two.product_ingredients.storeProductIngredientAjax") }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'Accept': 'application/json',
                ...
                async submitEditForm() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            `{{ url('sub_two/product_ingredients/ajax') }}/${this.editIngredientData.uuid}/update`, {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'Accept': 'application/json',
