...
<div x-data="{ deleteModal: false }" class="relative bg-white border border-[#4A2C1D] rounded-2xl p-4 flex flex-col">
    ...
    <div class="relative w-full h-48 bg-gray-100 rounded-xl overflow-hidden mb-4">
        <button
            @click.prevent="$dispatch('open-image-modal', {
                images: ['{{ $product->product?->product_img ?? $product->product_img }}'],
                index: 0
            })"
            ...
            <img src="{{ $product->product?->product_img
                ? asset('storage/app/public/' . $product->product->product_img)
                : ($product->product_img
                    ? asset('storage/app/public/' . $product->product_img)
                    : 'https://ui-avatars.com/api/?name=' .
                        urlencode($product->product?->product_name ?? ($product->product_name ?? 'N/A')) .
                        '&background=7F5539&color=FFFFFF') }}"
                alt="{{ $product->product?->product_name ?? ($product->product_name ?? 'N/A') }}"
                class="w-full h-full object-cover rounded-xl" />

            @if (!empty($product->product?->product_img ?? $product->product_img))
                <div
    ...
    <div class="flex flex-col space-y-2 mb-4">
        <p class="text-xs font-mono text-gray-500" title="Branch Name">
            {{ $product->branch->branch_name ?? ($product->product?->branch->branch_name ?? 'N/A') }}
        </p>

        {{-- ✅ Show the correct PBN depending on context --}}
        <p class="text-xs font-mono text-gray-500" title="Batch Number">
            {{ $type === 'damaged' ? $product->product?->product_batch_no ?? 'N/A' : $product->product_batch_no ?? 'N/A' }}
        </p>
        ...
        <div class="mb-4">
                ...
                @if ($type === 'archived')
                    <span
                        class="inline-block text-xs font-semibold px-2.5 py-1 rounded-full
                                 {{ $product->product_status == 1 ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
            ...
            <span
                ...
                {{ $product->product?->product_type ?? ($product->product_type ?? 'N/A') }}
            </span>
        </div>
        ...
        @if ($type === 'archived')
            <div class="flex items-center gap-2 text-gray-600">
                <span class="font-semibold text-[#4A2C1D]">Quantity:</span>
                <span class="text-[#7F5539] font-mono">
                    {{ number_format($product->quantity_in ?? 0) }} {{ $product->unit ?? '' }}
                </span>
                @if (($product->quantity_in ?? 0) <= ($product->quantity_threshold ?? 0))
                ...
                @else
                    <span class="text-sm text-[#7F5539]">
                        (Threshold: {{ number_format($product->quantity_threshold ?? 0) }}
                        {{ $product->unit ?? '' }})
                    </span>
                @endif
            </div>
            @if ($product->date_expiration)
                <div class="flex items-center gap-2 text-gray-600">
                    ...
                    <span class="text-[#7F5539] font-mono">
                        {{ \Carbon\Carbon::parse($product->date_expiration)->format('M d, Y, g:i A') }}
                    </span>
                </div>
            @endif
        @elseif ($type === 'expired')
        ...
        @elseif ($type === 'damaged')
            <div class="flex items-center gap-2 text-gray-600">
                ...
                <span class="text-[#7F5539] font-mono">
                    {{ number_format($product->quantity_out ?? 0) }}
                </span>
            </div>
            <div class="flex items-center gap-2 text-gray-600">
                <span class="font-semibold text-[#4A2C1D]">Reason:</span>
                <span class="text-[#7F5539] font-mono">{{ $product->reasons ?? 'N/A' }}</span>
            </div>
            <div class="flex items-center gap-2 text-gray-600">
                <span class="font-semibold text-[#4A2C1D]">Date Damaged:</span>
                <span class="text-[#7F5539] font-mono">
                    {{ \Carbon\Carbon::parse($product->date_damaged)->format('M d, Y, g:i A') }}
                </span>
            </div>
        @endif
    </div>
    ...
    <div class="border-t border-gray-100 pt-2 text-xs text-gray-500 space-y-1">
        @if ($product->created_by && $product->date_created)
            <div class="flex justify-between">
                <span>Created By: {{ $product->creator->first_name ?? '' }}
                    {{ $product->creator->last_name ?? '' }}</span>
                <span>{{ \Carbon\Carbon::parse($product->date_created)->format('M d, Y, g:i A') }}</span>
            </div>
        @endif
        @if ($product->last_updated_by && $product->last_date_updated)
            <div class="flex justify-between">
                <span>Last Updated By: {{ $product->last_updator->first_name ?? '' }}
                    {{ $product->last_updator->last_name ?? '' }}</span>
                <span>{{ \Carbon\Carbon::parse($product->last_date_updated)->format('M d, Y, g:i A') }}</span>
            </div>
        @endif
    </div>
    ...
    @if ($type === 'archived')
        <div x-show="deleteModal" x-transition
            ...
            style="display: none;">
            <form action="{{ route('sub_two.products.reactivateProduct', $product->uuid) }}" method="POST"
