...
<style>
    ...
    .branch-readonly .branch-icon {
        color: #6b7280;
    }

    .branch-readonly .branch-name {
        font-weight: 500;
    }
                    ...
                    <div>
                        ...
                        <div class="branch-readonly">
                            <div class="flex items-center gap-2">
                                <svg class="w-5 h-5 branch-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span class="text-xs text-gray-500">(Staff branch - read only)</span>
                        </div>
                        <input type="hidden" x-model="form.branch_id" value="{{ $branches->first()->id ?? '' }}">
                        <p class="mt-1 text-xs text-gray-500">Rewards are automatically assigned to your branch</p>
                    </div>
...
document.addEventListener('alpine:init', function() {
    Alpine.data('redeemableItemsData', () => ({
        ...
        // Init
        init() {
            // Set branch ID from server data
        ...
        async loadItems(page = 1) {
            ...
            try {
                ...
                const response = await fetch(`/sub_two/redeemable-items?${params.toString()}`, {
                    headers: {
        ...
        openAddModal() {
            ...
            this.form = {
                ...
                item_description: '',
                category: '',
                branch_id: this.branches && this.branches.length > 0 ? this.branches[0].id : '',
                is_active: 1
            };
        ...
        async openEditModal(id) {
            try {
                const response = await fetch(`/sub_two/redeemable-items/${id}/data`, {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                ...
                if (data.success) {
                    ...
                    this.form = {
                        ...
                        branch_id: item.branch_id || this.branches[0]?.id || '',
        ...
        async submitForm() {
            ...
            try {
                const url = this.isEditing 
                    ? `/sub_two/redeemable-items/${this.form.id}`
                    : '/sub_two/redeemable-items';
        ...
        async toggleStatus(id) {
            ...
            try {
                const response = await fetch(`/sub_two/redeemable-items/${id}/status`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        async deleteItem(id) {
            ...
            try {
                const response = await fetch(`/sub_two/redeemable-items/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
