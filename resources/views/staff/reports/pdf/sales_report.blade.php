...
<head>
    ...
    <style>
        * {
        ...
        /* Rounded-corner card wrapper for standalone tables (Sales
           Summary, Payment Methods, Popular Services, Products Sold),
...
<div class="summary-cards">
    ...
    <div class="card">
        ...
        <div class="value emerald">₱{{ number_format($salesData['total_net_revenue'] ?? 0, 2) }}</div>
    </div>
    <div class="card">
...
<table>
    <thead>
        <tr>
            ...
            <th style="width: 11%;" class="text-right">Booking Revenue</th>
            <th style="width: 11%;" class="text-right">Extension Revenue</th>
            <th style="width: 11%;" class="text-right">Order Revenue</th>
            <th style="width: 11%;" class="text-right">Reward Discount</th>
            <th style="width: 11%;" class="text-right">Total Revenue</th>
            <th style="width: 11%;" class="text-right">Net Revenue</th>
            <th style="width: 6%;" class="text-center">Bookings</th>
            <th style="width: 6%;" class="text-center">Orders</th>
    ...
    <tbody>
        @forelse($salesData['by_branch'] ?? [] as $branch)
            <tr>
                ...
                <td class="text-right" style="color: #dc2626;">₱{{ number_format($branch['reward_discount'] ?? 0, 2) }}</td>
                <td class="text-right" style="font-weight: bold;">₱{{ number_format($branch['total_revenue'] ?? 0, 2) }}</td>
                <td class="text-right" style="font-weight: bold; color: #059669;">₱{{ number_format($branch['net_revenue'] ?? 0, 2) }}</td>
    ...
    @if(count($salesData['by_branch'] ?? []) > 0)
        <tfoot>
            <tr class="grand-total">
                ...
                <td class="text-right" style="color: #dc2626;">₱{{ number_format($salesData['by_branch']->sum('reward_discount'), 2) }}</td>
                <td class="text-right" style="color: #7F5539; font-size: 10px;">₱{{ number_format($salesData['total_revenue'] ?? 0, 2) }}</td>
                <td class="text-right" style="color: #059669; font-size: 10px;">₱{{ number_format($salesData['total_net_revenue'] ?? 0, 2) }}</td>
                                        ...
                                        @if($loop->first)
                                            ...
                                            <td>
                                                @if($orderDate)
                                                    ...
                                                    <div class="text-muted" style="font-size: 8px;">{{ $orderDate->format('h:i A') }}</div>
                                                @else
                                                    <div>{{ $order['date'] ?? '—' }}</div>
                                                @endif
                                            </td>
