...
@section('content')
<div x-data="feedbackReportData()" x-init="init()" class="p-4">
    ...
    <div class="bg-[#7F5539]/5 border border-[#7F5539]/20 rounded-lg px-4 py-3 mb-6 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-500">Showing feedback for your assigned branch</span>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600">Your Branch</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ $branch->branch_name ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Average star ratings and feedback volume for your branch</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Average ratings per service type for your branch</p>
        </div>
                            ...
                            <div class="border border-gray-200 rounded-lg overflow-hidden">
                                <div class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-200">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs font-semibold text-gray-700">Customer Comments</span>
                                    </div>
                                    <span class="text-xs text-gray-400" x-text="cat.comments.length + ' comment(s)'"></span>
                                </div>
                                <div class="px-4 py-3 max-h-64 overflow-y-auto">
                                    <div x-show="!cat.comments.length" class="text-sm text-gray-400 italic text-center py-4">
                                        No written comments available for this category.
                                    </div>
                                    <template x-for="comment in cat.comments" :key="comment">
                                        <div class="py-2 border-b border-gray-100 last:border-0">
                                            <p class="text-sm text-gray-700 leading-relaxed" x-text="comment"></p>
                                        </div>
                                    </template>
                                </div>
                            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('feedbackReportData', () => ({
            byBranch:     [],
            byCategory:   [],
            filters: {
                date_from: '',
            ...
            async fetchReport() {
                ...
                try {
                    ...
                    const res  = await fetch(`{{ route('sub_two.reports.feedback_report') }}?${params}`, {
            ...
            exportPDF() {
                ...
                window.open(`{{ route('sub_two.reports.export_feedback_pdf') }}?${params}`, '_blank');
            },
        }));
    });
</script>
