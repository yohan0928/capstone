                                    ...
                                    <div class="flex items-center gap-2 min-h-[40px] w-full">
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <div class="flex-1 min-w-[70px]">
                                                <a :href="`/sub_two/booking_lists/details/${booking.uuid}`"
                                            ...
                                            <!-- Main Payment Button - Shows for checked out (checkin_status === 0) when main payment is unpaid -->
                                            <div class="flex-1 min-w-[70px]"
                                                x-show="booking.checkin_status === 0 && booking.payment?.payment_status !== 1">
                                                <a :href="`/sub_two/booking_lists/main-payment/${booking.uuid}`"
                                            ...
                                            <!-- Extension Payment Button - Shows for checked out (checkin_status === 0) when extension time used AND extension payment is unpaid -->
                                            <div class="flex-1 min-w-[70px]"
                                                x-show="booking.checkin_status === 0 && booking.extended_time_used > 0 && booking.extension_payment?.payment_status !== 1">
                                                <a :href="`/sub_two/booking_lists/extension-payment/${booking.uuid}`"
                                                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-xs font-medium whitespace-nowrap">
                                                    Ext Pay
                                                </a>
                                            </div>

                                            <!-- Order Payment Button - Shows when checked out (checkin_status === 0), main AND extension payments are paid, and there are unpaid pay-later orders -->
                                            <div class="flex-1 min-w-[70px]"
                                                x-show="hasUnpaidPayLaterOrders(booking)">
                                                <a :href="`/sub_two/booking_lists/order-payment/${booking.uuid}`"
        ...
        <div x-show="showConfirmModal" x-cloak @click.self="closeConfirmModal()"
            ...
            <div class="flex items-center justify-center min-h-screen p-4 h-[90vh]">
                <div class="bg-white rounded-lg shadow-xl w-full max-w-md relative">
                    ...
                    <form method="POST" action="{{ route('sub_two.booking_lists.confirmBooking') }}"
        ...
        <div x-show="showNoShowModal" x-cloak @click.self="closeNoShowModal()"
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="bg-white rounded-lg shadow-xl w-full max-w-md relative text-center">
                    <form method="POST" action="{{ route('sub_two.booking_lists.markNoShow') }}">
                        @csrf
                        <input type="hidden" name="booking_id" :value="selectedBooking?.id">
                ...
                async updateFilterOptions() {
                    try {
                        ...
                        const response = await fetch(
                            `/staff/booking-lists/filter-options?${queryParams.toString()}`, {
                                method: 'GET',
                                headers: {
            ...
            Alpine.data('bookingListPage', () => ({
                ...
                async submitNote() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            '{{ route('sub_two.booking_lists.updateNote') }}', {
                                method: 'POST',
                                headers: {
                ...
                async loadNotes(bookingId) {
                    try {
                        ...
                        // Use the correct route for staff
                        const response = await fetch(`/sub_two/booking_lists/notes/${bookingId}`, {
                            method: 'GET',
                            headers: {
                                'Accept': 'application/json',
                                'X-Requested-With': 'XMLHttpRequest',
