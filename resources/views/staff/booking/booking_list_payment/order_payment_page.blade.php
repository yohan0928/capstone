...
@section('title', 'Order Payment - Booking #' . $booking->booking_ref_no)
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <!-- Back Link -->
        <a href="{{ route('sub_two.booking_lists.showBookingList') }}" class="inline-flex items-center gap-2 text-emerald-900 hover:text-emerald-700 font-semibold mb-6 sm:mb-8 transition-colors duration-200">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <!-- Header -->
            <div class="bg-gradient-to-r from-emerald-800 to-emerald-600 text-white px-4 sm:px-6 py-6 sm:py-8">
                <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-bold mb-2">Order Payment</h1>
                        <p class="text-white/90 text-sm sm:text-base">Booking Reference: {{ $booking->booking_ref_no }}</p>
                        <p class="text-white/90 text-sm sm:text-base">Payment Category: Orders (Pay Later)</p>
                    </div>
            ...
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 sm:p-6 border-l-4 border-emerald-600">
                        ...
                        <div class="space-y-2 sm:space-y-3">
                            ...
                            <div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                ...
                                <span class="font-semibold text-emerald-700 truncate">{{ $booking->customerAccount->email ?? 'N/A' }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="font-semibold text-gray-700">Phone:</span>
                                <span class="font-semibold text-emerald-700">{{ $booking->customerAccount->contact_no ?? 'N/A' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Booking Details -->
                    <div class="bg-gray-50 rounded-lg p-4 sm:p-6 border-l-4 border-emerald-600">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Booking Details</h3>
                        <div class="space-y-2 sm:space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <span class="font-semibold text-gray-700">Branch:</span>
                                <span class="font-semibold text-emerald-700">{{ $booking->branch->branch_name ?? 'N/A' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <span class="font-semibold text-gray-700">Booking Date:</span>
                                <span class="font-semibold text-emerald-700">{{ \Carbon\Carbon::parse($booking->booking_date)->format('M j, Y') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="font-semibold text-gray-700">Booking Status:</span>
                    ...
                    <!-- Payment Summary -->
                    <div class="bg-gray-50 rounded-lg p-4 sm:p-6 border-l-4 border-emerald-600">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Payment Summary</h3>
                        <div class="space-y-2 sm:space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <span class="font-semibold text-gray-700">Total Unpaid Orders:</span>
                                <span class="font-semibold text-emerald-700">{{ $unpaidOrders->count() }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="font-semibold text-gray-700">Total Amount Due:</span>
                                <span class="text-xl font-bold text-emerald-700">₱{{ number_format($totalUnpaidOrdersAmount, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 sm:p-6 mb-6 sm:mb-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="bg-white border border-emerald-200 rounded-lg p-4 order-item">
                                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-3">
                                    <div>
                                        <h4 class="font-semibold text-gray-900">Order #{{ $order->order_ref_no }}</h4>
                                        <p class="text-sm text-gray-600">Date: {{ \Carbon\Carbon::parse($order->order_date)->format('M j, Y g:i A') }}</p>
                                    </div>
                                    <div class="text-right">
                                        ...
                                        <p class="text-lg font-bold text-emerald-700 mt-1">
                                            ₱{{ number_format($order->payments->first()->total_amount ?? $order->items->sum('sub_total'), 2) }}
                                        </p>
                                    </div>
                                </div>
                        ...
                        <!-- Summary -->
                        <div class="bg-emerald-100 border border-emerald-300 rounded-lg p-4 mt-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center md:text-left">
                                <div>
                                    <p class="text-sm text-emerald-700">Total Unpaid Orders</p>
                                    <p class="text-lg font-semibold text-emerald-900">{{ $unpaidOrders->count() }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-emerald-700">Orders Total</p>
                                    <p class="text-lg font-semibold text-emerald-900">₱{{ number_format($totalUnpaidOrdersAmount, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-emerald-700">Payment Status</p>
                ...
                <div class="bg-gray-50 rounded-lg p-4 sm:p-6 lg:p-8">
                    ...
                    <form method="POST" action="{{ route('sub_two.booking_lists.updateOrderPayment') }}" id="paymentForm">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                            ...
                            <div class="space-y-4 sm:space-y-6">
                                <div>
                                    ...
                                    <select name="payment_method" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200" required id="paymentMethod">
                            ...
                            <div class="space-y-4 sm:space-y-6">
                                <div>
                                    ...
                                    <input type="number" name="amount_paid" step="0.01" 
                                           min="{{ $totalUnpaidOrdersAmount }}" 
                                           value="{{ $totalUnpaidOrdersAmount }}" 
                                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200" 
                                    ...
                                    <p class="text-sm text-gray-500 mt-2">
                                        Total due: ₱{{ number_format($totalUnpaidOrdersAmount, 2) }}
                                    </p>
                                    <!-- Warning Message -->
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-end mt-6 sm:mt-8">
                            ...
                            <button type="submit" id="submitButton"
                                    class="inline-flex items-center justify-center px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition-colors duration-200 order-1 sm:order-2 disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const totalDue = {{ $totalUnpaidOrdersAmount }};
