...
@section('content')
    ...
    <div x-data="bookingListPage()" x-init="init()" class="p-4">
        <!-- Header -->
        <h1 class="text-2xl font-bold text-gray-900 mb-8 text-center">My Bookings</h1>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-gray-900"
                                                x-text="booking.booking_ref_no || 'N/A'">
                                            </div>
                                        </div>
                                    </div>
                                </td>
...
<td class="px-6 py-4 whitespace-nowrap">
    <div class="flex items-center gap-2 min-h-[40px] w-full">
        <!-- Left side - Larger View and Notes buttons -->
        <div class="flex flex-1 gap-2">
            <!-- View Button - Now wider -->
            <div class="flex-1">
                <a :href="`/sub_three/my_bookings/details/${booking.uuid}`"
                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-[#7F5539] text-white rounded-lg hover:bg-[#4A2C1D] transition-colors text-xs font-medium">
                    View
                </a>
            </div>
    
            <!-- Add Note Button - Now wider -->
            <div class="flex-1">
                <button @click="openNoteModal(booking); addBodyClass()"
                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-xs font-medium">
                    Notes
                </button>
            </div>

            <!-- Re-sched Button - ONLY SHOWS WHEN booking_status is 3 (no-show) -->
            <div class="flex-1"
                x-show="booking.booking_status == 3">
                <a :href="`/sub_three/my_bookings/reschedule/${booking.uuid}`"
                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-xs font-medium">
                    Re-sched
                </a>
            </div>
            ...
            <div class="flex-1"
                ...
                <button @click="openNoShowModal(booking); addBodyClass()"
                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-xs font-medium">
                    No Show
                </button>
            </div>
            ...
            <!-- Rating Button -->
            <div class="flex-1"
                x-show="booking.booking_status == 4">
                <a :href="`/sub_three/my_bookings/feedback/${booking.uuid}`"
                    class="inline-flex items-center justify-center w-full px-4 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-xs font-medium">
                    Rate
                </a>
            </div>
        </div>
        ...
        <div x-show="showNoShowModal" x-cloak @click.self="closeNoShowModal()"
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="bg-white rounded-lg shadow-xl w-full max-w-md relative text-center">
                    <form method="POST" action="{{ route('sub_three.my_bookings.markNoShow') }}">
                        @csrf
                ...
                async updateFilterOptions() {
                    try {
                        ...
                        const response = await fetch(
                            `/sub_three/booking-lists/filter-options?${queryParams.toString()}`, {
                                method: 'GET',
                                headers: {
            ...
            Alpine.data('bookingListPage', () => ({
                ...
                async submitNote() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            '{{ route('sub_three.my_bookings.updateNote') }}', {
                                method: 'POST',
                                headers: {
                ...
                async loadNotes(bookingId) {
                    try {
                        ...
                        const response = await fetch(`/sub_three/my_bookings/notes/${bookingId}`, {
                            method: 'GET',
                            headers: {
                                'Accept': 'application/json',
                                'X-Requested-With': 'XMLHttpRequest',
