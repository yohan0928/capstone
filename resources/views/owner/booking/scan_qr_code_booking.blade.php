...
@section('content')
    ...
    <script>
        ...
        <div class="bg-white border rounded-xl p-6 mb-6 {qrValidityClass}" id="booking-card">
            ...
            <!-- QR Code Validity Status - Simplified -->
            <div class="mb-4">
                {qrValidityStatus}
            </div>
        ...
        async function fetchBookingInfo(bookingRef) {
            ...
            try {
                ...
                const response = await fetch(
                    '{{ route('sub_one.scan_qr_code_bookings.getBookingByBookingRefNo') }}?booking_ref=' +
        ...
        function displayBookingInfo(booking) {
            ...
            // ================================================================
            // ORDER ELIGIBLE BADGE - Shows if customer is checked in OR booked
            // ================================================================
            ...
            // ================================================================
            // QR CODE VALIDITY STATUS - Simplified and permissive
            // ================================================================
            const isQrCodeValid = booking.is_qr_code_valid === true;
            // Removed red border styling entirely when ordering is allowed (i.e. checked-in)
            ...
            if (isQrCodeValid) {
                ...
                // Completely removed the active red error message banner
                qrValidityStatus = '';
            }

            // ================================================================
            // EXPIRED NOTICE - FOR DISPLAY ONLY
            // ================================================================
            ...
            // ================================================================
            // ACTION BUTTON LOGIC - For check-in/check-out
            // ================================================================
            ...
            // ================================================================
            // ORDER BUTTON - MODIFIED: Shows if customer is checked in
            // checkin_status = 1 (checked-in)
            // ================================================================
            ...
            // Completely removed visual card red borders & background injection to satisfy cleanup requests
        }
        ...
        async function proceedToCheckout(bookingId, checkinId, bookingRefNo, bookingUuid) {
            try {
                ...
                const routeUrl = '{{ route('sub_one.scan_qr_code_bookings.checkout') }}';
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        if (data.redirect_url) {
                            ...
                            window.location.href = '{{ route('sub_one.booking_lists.showBookingList') }}';
                        }
                    }, 2000);
        ...
        async function redirectToPos(bookingId, checkinId) {
            try {
                ...
                const csrfToken = csrfMeta.getAttribute('content');
                const routeUrl = '{{ route('sub_one.scan_qr_code_bookings.redirectToPos') }}';
        ...
        function redirectToCheckinList(bookingRefNo) {
            const url = '{{ route('sub_one.customer_checkins.index') }}?brn=' + encodeURIComponent(bookingRefNo);
        ...
        async function handleOnlineCheckin(bookingId, bookingRefNo) {
            try {
                ...
                const csrfToken = csrfMeta.getAttribute('content');
                const routeUrl = '{{ route('sub_one.scan_qr_code_bookings.storeCheckin') }}';
