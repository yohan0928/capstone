...
@section('content')
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('checkinData', () => ({
                ...
                async clearBrnFilter() {
                    ...
                    try {
                        const clearUrl =
                            `{{ route('sub_one.customer_checkins.index') }}?clear_brn=true&ajax=true`;
                ...
                async showExtendTimeModal(checkin) {
                    ...
                    try {
                        ...
                        const url = `/sub_one/customer_checkins/extend-time-modal/${checkinId}`;
                ...
                async processTimeExtension() {
                    ...
                    try {
                        ...
                        form.method = 'POST';
                        form.action =
                            `/sub_one/customer_checkins/extend-time/${this.currentExtendCheckinId}`;
    ...
    try {
        ...
        const url = `{{ route('sub_one.customer_checkins.index') }}?${queryParams.toString()}`;
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
    ...
    try {
        ...
        const clearUrl = `{{ route('sub_one.customer_checkins.index') }}?clear_brn=true&ajax=true`;
        ...
        this.isLoading = false;
                ...
                async confirmCheckout(checkinId) {
                    try {
                        const response = await fetch(
                            `/sub_one/customer_checkins/update-status/${checkinId}`, {
                                method: 'PATCH',
                                headers: {
                ...
                async addOrder(checkin) {
                    try {
                        ...
                        const posUrl = `{{ route('sub_one.pos.index') }}?${params.toString()}`;
