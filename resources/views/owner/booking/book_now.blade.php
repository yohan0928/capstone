...
@section('content')
    <div class="max-w-6xl mx-auto p-4 md:p-6" x-data="bookingForm()">
        <!-- Header Section -->
        <div class="text-center mb-8">
            <h1 class="text-4xl sm:text-5xl font-serif font-bold text-coffee-900 mb-3 mt-4">
                Book Now
            </h1>
        ...
        <form id="bookingForm" method="POST" action="{{ route('sub_one.book_now.store') }}" class="space-y-8">
            @csrf
            ...
            <div class="bg-coffee-50 rounded-2xl border border-coffee-100 p-6 sm:p-8 mb-4 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Branch -->
                    <div>
                        <label for="branch_id" class="block text-sm font-medium text-coffee-900 mb-2">
                        ...
                        <select
                            ...
                            id="branch_id" name="branch_id" x-model="selectedBranch" @change="onBranchChange()" required>
                            <option value="" disabled>Choose your preferred branch</option>
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" data-open-time="{{ $branch->open_time ?? '11:00' }}"
                                    data-close-time="{{ $branch->close_time ?? '07:00' }}">
                                    {{ $branch->branch_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            @change="onCategoryChange()" :disabled="!selectedBranch" required>
                            <option value="" disabled x-text="selectedBranch ? 'Select a category' : 'Select a branch first'"></option>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required>
                            <option value="" disabled x-text="selectedBranch && selectedCategory ? 'Select a seat/room' : 'Select branch and category first'"></option>
    ...
    <!-- ===== ALPINE.JS BOOKING FORM WITH REWARDS ===== -->
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('bookingForm', () => ({
                ...
                branchData: @json($branches->keyBy('id')),
                ...
                async loadCustomerRewards() {
                    ...
                    try {
                        const response = await fetch('{{ route("sub_one.book_now.get_customer_rewards") }}', {
                            method: 'POST',
                            headers: {
                ...
                toggleRewardSelection(reward) {
                    ...
                    if (this.selectedReward && this.selectedReward.id === reward.id) {
                        ...
                        if (reward.reward_type === 'free_service' && reward.service_category_id && reward.target_service_id) {
                            ...
                            // Load services for this category manually so we don't let default Alpine logic override our choice
                            ...
                            // Check if our targeted service exists in our available hourly services
                            ...
                            if (targetServiceExists) {
                                ...
                                // Fallback to first available if targeted item is not matching hourly regex constraints
                ...
                async applySelectedReward() {
                    ...
                    try {
                        const response = await fetch('{{ route("sub_one.book_now.apply_reward") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                async loadReturningCustomers() {
                    try {
                        const response = await fetch('{{ route('sub_one.book_now.get_returning_customers') }}');
                        const data = await response.json();
                ...
                selectNewCustomer() {
                    if (this.validateModalCustomer()) {
                        ...
                        // New customers don't have rewards yet
                        this.availableRewards = [];
                        this.selectedReward = null;
                        this.appliedReward = null;
                        this.customerRewardId = null;
                ...
                async updateAvailableSeats() {
                    ...
                    if (this.selectedBranch && this.selectedCategory) {
                        try {
                            const response = await fetch("{{ route('sub_one.book_now.get_available_seats') }}", {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
