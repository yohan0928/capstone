...
@section('title', 'Archived Service Names')

@section('content')
    <div x-data="archivedServiceNameData()" x-init="init()" class="p-4">
        <!-- Header Section -->
            ...
            <div class="text-center lg:text-left mb-4 lg:mb-0">
                <h1 class="text-2xl font-bold text-[#4A2C1D]">
                    ...
                    <span class="block lg:inline text-lg font-semibold text-[#7F5539] lg:ml-2">
                        {{ $serviceCategory->service_category }} (Archived)
                    </span>
                </h1>
                <p class="text-sm text-gray-500 mt-1">Archived services will be permanently deleted after 30 days</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <div class="hidden sm:flex items-center justify-between mb-4">
                    <!-- Left: Header -->
                    <h2 class="text-lg font-semibold text-gray-900">Archived Service Names</h2>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"
                                        x-text="'₱' + service.price.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})">
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-wrap gap-1 max-w-[200px]">
                                        <span
                                            ...
                                            <span x-text="service.space_type"></span>
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex justify-center items-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                            :class="getArchivedStatusClasses(service.service_name_status)">
                                            <span x-text="getArchivedStatusText(service.service_name_status)"></span>
                                        </span>
                                    </div>
                                </td>
                                ...
                                <!-- Archived Date -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 text-center" x-text="formatDate(service.date_updated || service.date_created)"></div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex justify-center items-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                            :class="getDaysLeftClass(service.days_left)">
                                            <span x-text="service.days_left + ' days'"></span>
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <div class="relative group">
                                            <form :id="'reactivate-service-' + service.uuid"
                                                :action="'{{ url('sub_one/service_names/reactivate') }}/' + service.uuid" method="POST"
                                                class="hidden">
                                                @csrf
                                                @method('PATCH')
                                            </form>
                                            <button @click="confirmReactivate(service)"
                            ...
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    ...
                                    <h5 class="text-sm font-medium text-gray-900"
                                        x-text="currentFilters.search ? 'No archived services found' : 'No archived services found'">
                                    </h5>
                                    <p class="text-sm text-gray-500"
                                        x-text="currentFilters.search ? 'Try adjusting your search terms.' : 'All archived services will appear here.'">
                                    </p>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div>
                        ...
                        <div class="mt-3 text-center sm:mt-5">
                            ...
                            <div class="mt-4">
                                ...
                                <p class="text-xs text-gray-400">This will make the service available again.</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            // Main component
            Alpine.data('archivedServiceNameData', () => ({
                // Initial state
                serviceNames: @json($serviceNames->items() ?? []),
                pagination: @json($serviceNames->toArray()),
                stats: @json($stats ?? []),
                currentFilters: {
                ...
                calculateDaysLeft() {
                    this.serviceNames.forEach(service => {
                        const archiveDate = service.date_updated || service.date_created;
                        if (archiveDate) {
                            try {
                                const archivedDate = new Date(archiveDate);
                                const today = new Date();
                                const daysSinceArchived = Math.floor((today - archivedDate) / (1000 * 60 * 60 * 24));
                                service.days_left = Math.max(0, 30 - daysSinceArchived);
                            } catch (error) {
                                console.error('Error calculating days left:', error);
                                service.days_left = 30; // Default
                            }
                        } else {
                            service.days_left = 30; // Default
                        }
                ...
                confirmReactivateAction() {
                    ...
                    const form = document.getElementById(`reactivate-service-${this.selectedService.uuid}`);
                    if (form) form.submit();
                },
