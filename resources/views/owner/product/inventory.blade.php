...
@section('content')
    <div x-data="inventoryData()" x-init="init()" class="p-4">
        ...
        {{-- SUMMARY STAT CARDS
             Beginning Balance | New Arrivals | Low Stock Alerts | Ending Balance
        --}}
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-8" id="stockLevelsSection">
            ...
            {{-- PAGINATION: "Showing X to Y of Z entries" + numbered pages --}}
        ...
        {{-- STOCK TRANSACTIONS (pending + recently resolved) --}}
                        ...
                        <tr x-show="!pagedTransactions.length">
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    ...
                                    <p class="text-sm text-gray-500">Stock-out declarations and their outcomes will show up here.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div x-show="txnTotalPages > 1" class="px-4 sm:px-6 py-4 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="text-sm text-gray-700">
                        ...
                        of <span x-text="pendingTransactions.length"></span> entries
                    </div>
                    <div class="flex items-center gap-1">
                            ...
                            <div class="relative p-3 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
                                ...
                                <div x-data="{
                                    ...
                                    get selectedBranchName() {
                                        if (!item.branch_id) return 'Select a branch';
                                        const branches = {{ Js::from($branches->map->only(['id', 'branch_name'])) }};
                                        const b = branches.find(b => b.id == item.branch_id);
                                        return b ? b.branch_name : 'Select a branch';
                                    },
                                    selectBranch(id) { item.branch_id = id; this.open = false; }
                                }" class="relative">
                                    <label class="block text-sm font-medium text-[#4A2C1D]">Branch</label>
                                    <button @click="open = !open" @click.away="open = false" type="button"
                                        ...
                                        <span x-text="selectedBranchName" :class="{ 'text-gray-500': !item.branch_id }"></span>
                                    ...
                                    <div x-show="open" x-transition class="absolute mt-1 w-full bg-white rounded-md shadow-lg z-20 border border-gray-200 max-h-60 overflow-y-auto" style="display:none;">
                                        @forelse ($branches as $branch)
                                            <a href="#" @click.prevent="selectBranch({{ $branch->id }})" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ $branch->branch_name }}</a>
                                        @empty
                                    ...
                                    <div>
                                        <div class="grid border-b border-gray-100 last:border-b-0 text-sm"
                                            ...
                                            <div class="px-4 py-3 flex flex-col gap-0.5 min-w-0">
                                                <span class="font-medium text-gray-700 break-words" x-text="item.product_name"></span>
            ...
            <div class="relative z-10 w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-xl shadow-xl flex flex-col max-h-[92dvh] sm:max-h-[85vh]"
                ...
                <div class="px-6 pt-6 pb-6 flex flex-col gap-4">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500">This will deduct <strong x-text="selectedTransaction?.items_count"></strong> item(s) from inventory.</p>
                    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('inventoryData', () => ({
                ...
                // Stock Transactions pagination getters
                get txnTotalPages() {
                    return Math.max(1, Math.ceil(this.pendingTransactions.length / this.txnPerPage));
                },
                get pagedTransactions() {
                    const start = (this.txnPage - 1) * this.txnPerPage;
                    return this.pendingTransactions.slice(start, start + this.txnPerPage);
                },
                get txnShowingFrom() {
                    return this.pendingTransactions.length ? (this.txnPage - 1) * this.txnPerPage + 1 : 0;
                },
                get txnShowingTo() {
                    return Math.min(this.txnPage * this.txnPerPage, this.pendingTransactions.length);
                },
                get txnPageNumbers() {
                ...
                stockInItems: [{ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, note: '' }],
                stockOutItems: [{ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, reason: '', note: '' }],
                ...
                init() {
                    ...
                    if (branchId) {
                        // Match against known branches to avoid setting an invalid filter
                ...
                openStockInModal() {
                    this.stockInItems = [{ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, note: '' }];
                    this.showStockInModal = true; this.addBodyClass();
                },
                closeStockInModal() { this.showStockInModal = false; this.removeBodyClass(); },
                addStockInItem()    { this.stockInItems.push({ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, note: '' }); },
                ...
                openStockOutModal() {
                    this.stockOutItems = [{ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, reason: '', note: '' }];
                    this.showStockOutModal = true; this.addBodyClass();
                },
                closeStockOutModal() { this.showStockOutModal = false; this.removeBodyClass(); },
                addStockOutItem()    { this.stockOutItems.push({ item_type: 'product', branch_id: '', product_id: '', ingredient_id: '', quantity: 1, reason: '', note: '' }); },
