...
@section('content')
    <!-- Header -->
    <h1 class="text-2xl font-bold text-gray-900 mt-4 mb-8 text-center">Reward Tiers</h1>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                        :class="tier.reward_type === 0 ?
                                            'bg-blue-100 text-blue-800' :
                                            'bg-purple-100 text-purple-800'">
                                        <span x-text="getBookingTypeLabel(tier.reward_type)"></span>
                                    </span>
                                </td>
                                <!-- 3. Reward Required -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900" x-text="tier.reward_required"></div>
                                </td>
                            ...
                            <div class="mt-4 space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 text-left mb-2">Reward
                                        Type</label>
                                    <select x-model="tierForm.reward_type"
                                        class="block w-full border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 px-3 py-2">
                                        <option value="" disabled>All Reward Types</option>
                                        <option value="1">Service</option>
                                        <option value="0">Product</option>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 text-left mb-2">Reward
                                        Required</label>
                                    <input type="number" x-model="tierForm.reward_required" min="1"
                                        class="block w-full border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 px-3 py-2"
                                        placeholder="Enter number of reward required">
                                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('filterState', () => ({
                filters: {
                    reward_type: '{{ request('reward_type', '1') }}',
            ...
            Alpine.data('rewardTierData', () => ({
                ...
                get activeFilters() {
                    ...
                    if (this.currentFilters.reward_type) {
                        filters.push({
                            key: 'reward_type',
                            label: `Type: ${this.getBookingTypeLabel(this.currentFilters.reward_type)}`
                        });
                    }
    ...
    this.tierForm = {
        ...
        reward_type: tier.reward_type.toString(),
    ...
    try {
        const url = this.isEditing ?
            `/sub_one/reward_tiers/${this.currentTierId}` :
            '/sub_one/reward_tiers';
        ...
        const formData = {
            ...
            reward_type: parseInt(this.tierForm.reward_type),
            reward_required: parseInt(this.tierForm.reward_required),
        ...
        // Don't close modal on error
                ...
                async toggleStatus(tierId, newStatus) {
                    ...
                    try {
                        const response = await fetch(`/sub_one/reward_tiers/${tierId}/status`, {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
